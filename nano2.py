its not using the specific prompt for newly added prompt as each themes name like "Photoshoot_with_bike, Selfie_with_ Cristiano_Ronaldo, but its not using the prompt in theme image generation "import os
import uuid
import base64
...
# ==================================================
# GLOBAL SUBJECT PROMPTS (GENDER / IDENTITY)
# ==================================================
SUBJECT_PROMPTS = {
    "male": (
...
# ==================================================
# THEME SCENE PROMPTS (SCENE / STYLE ONLY)
# ==================================================
THEME_SCENES = {
   ...
   "Photoshoot_with_bike": (
    ...
    "Selfie_with_ Cristiano_Ronaldo": (
...
TARGET_WIDTH = 1080
TARGET_HEIGHT = 1350  # 4:5 portrait
...
# ==================================================
# ADVANCED GENDER DETECTION (HAIR + BEARD LOGIC)
# ==================================================
def detect_gender(face_bytes: bytes):
    ...
    if beard_detected and male_prob > 0.6:
        return "male", max(male_prob, 0.85)
    elif long_hair and female_prob > 0.4:
        return "female", max(female_prob, 0.75)
    elif male_prob > 0.8:
        return "male", male_prob
    elif female_prob > 0.8:
        return "female", female_prob

    return "neutral", max(male_prob, female_prob)
...
def generate_with_nano_banana(
    face_bytes: bytes,              # processed face
    original_face_bytes: bytes,     # raw upload
    theme_name: str,
    themes_root: str
):
    ...
    if not image_bytes:
        raise NanoBananaError("NO_IMAGE", "No image returned")
    ...
    with open(os.path.join(theme_dir, filename), "wb") as f:
        f.write(image_bytes)

    print("🍌 Nano Banana → SUCCESS")
